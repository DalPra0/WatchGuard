...
def video_processing():
    ...
    print("A iniciar a webcam do PC...")

    cap = cv2.VideoCapture(VIDEO_SOURCE)

    if not cap.isOpened():
        print(f"ERRO FATAL: Não foi possível abrir a webcam.")
        return

    print("Webcam iniciada com sucesso. A iniciar processamento.")
    previous_frame = None
    is_recording = False
    ...
    while True:
        ...
        if motion_detected and not is_recording:
            ...
            add_log("Camera", {"message": "Movimento detectado (Webcam)", "video_file": filename})
...
def serve_recording(filename):
    ...
    if not range_header:
        # Se não houver pedido de "range", envia o ficheiro inteiro
    ...
    if not m:
        return "Range header inválido", 400
...
if __name__ == '__main__':
    ...
    load_logs()
    add_log("System", "Servidor iniciado em modo WEBCAM.")
